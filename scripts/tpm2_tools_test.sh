...
for t in tpm2_startup tpm2_getcap tpm2_getrandom tpm2_pcrread \
         ...
         tpm2_policyor tpm2_policysecret tpm2_policyauthorize; do
...
run_test "policylocality (locality 0)" \
    tpm2_policylocality -S "$TEST_TMPDIR/loc_session.ctx" 0
...
if check_tool tpm2_import; then
    ...
    if check_tool tpm2_duplicate; then
        ...
        tpm2_flushcontext "$TEST_TMPDIR/import_key.ctx" 2>/dev/null || true
    else
...
if check_tool tpm2_makecredential && check_tool tpm2_activatecredential; then
    # Flush transient objects from prior tests to free handle slots
    for h in $(tpm2_getcap handles-transient 2>/dev/null | grep "^-" | sed 's/- //'); do
        tpm2_flushcontext "$h" 2>/dev/null || true
    done

    # Create EK (endorsement key) and AK (attestation key)
    run_test "createprimary EK for credential" \
        tpm2_createprimary -C e -g sha256 -G rsa \
            -c "$TEST_TMPDIR/ek.ctx"

    run_test "createprimary SRK for credential" \
        tpm2_createprimary -C o -g sha256 -G rsa \
            -c "$TEST_TMPDIR/cred_srk.ctx"

    run_test "create AK under SRK" \
        tpm2_create -C "$TEST_TMPDIR/cred_srk.ctx" \
            -g sha256 -G rsa:rsassa:null \
            -u "$TEST_TMPDIR/ak.pub" -r "$TEST_TMPDIR/ak.priv" \
            -a "restricted|sign|fixedtpm|fixedparent|sensitivedataorigin|userwithauth"

    run_test "load AK" \
        tpm2_load -C "$TEST_TMPDIR/cred_srk.ctx" \
            -u "$TEST_TMPDIR/ak.pub" -r "$TEST_TMPDIR/ak.priv" \
            -c "$TEST_TMPDIR/ak.ctx"

    # Get EK public key and AK name for MakeCredential
    tpm2_readpublic -c "$TEST_TMPDIR/ek.ctx" \
        -o "$TEST_TMPDIR/ek.pub" > /dev/null 2>&1
    tpm2_readpublic -c "$TEST_TMPDIR/ak.ctx" \
        -n "$TEST_TMPDIR/ak.name" > /dev/null 2>&1

    # MakeCredential (challenge) using EK public
    echo -n "secret-credential!" > "$TEST_TMPDIR/cred_secret.bin"

    run_test "makecredential" \
        tpm2_makecredential -u "$TEST_TMPDIR/ek.pub" \
            -s "$TEST_TMPDIR/cred_secret.bin" \
            -n "$(xxd -p -c 256 "$TEST_TMPDIR/ak.name")" \
            -o "$TEST_TMPDIR/cred_blob.bin"

    # ActivateCredential (response) using AK + EK
