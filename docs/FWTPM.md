...
103 of 113 commands from the TPM 2.0 v1.38 specification (91% coverage). The
...
### wolfTPM Examples (`run_examples.sh`)

Runs all wolfTPM example programs against fwtpm_server. Start the server
first, then run the examples:

```sh
# Socket mode (--enable-swtpm)
rm -f fwtpm_nv.bin
src/fwtpm/fwtpm_server --port 2321 --platform-port 2322 &
WOLFSSL_PATH=../wolfssl ./examples/run_examples.sh

# TIS mode (--enable-fwtpm only)
...
Runs the wolfTPM unit test suite against fwtpm_server:
...
### tpm2-tools Compatibility (`scripts/tpm2_tools_test.sh`)

Tests TPM simulator compatibility with `tpm2-tools` (standard open-source
TPM 2.0 command-line utilities) using the mssim TCTI.

```sh
# Start server, then run tpm2-tools tests
scripts/tpm2_tools_test.sh

# Or with server already running
scripts/tpm2_tools_test.sh --no-start

# Override TCTI type (default: mssim)
scripts/tpm2_tools_test.sh --tcti=mssim
```

Requires: `tpm2-tools >= 5.0`, `libtss2-tcti-mssim`
...
Because `CreatePrimary` generates keys using the wolfCrypt RNG (not a
KDFa-derived deterministic process from hierarchy seeds), the fwTPM caches
primary keys by hierarchy and template hash. This ensures that creating the
same key type under the same hierarchy always returns the same key material.

- Cache key: SHA-256 hash of the `TPMT_PUBLIC` template
- Cache size: `FWTPM_MAX_PRIMARY_CACHE` (default 16) slots
- Covers all standard hierarchy/template combinations used by wolfTPM examples
