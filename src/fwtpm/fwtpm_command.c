...
static TPM_RC FwParseSensitiveCreate(TPM2_Packet* cmd, int cmdSize,
    ...
    TPM2_Packet* rsp, UINT16 cmdTag)
{
    ...
    if (rc == 0) {
        if (startupType == TPM_SU_CLEAR) {
            ...
            wc_RNG_GenerateBlock(&ctx->rng, ctx->nullSeed,
                FWTPM_SEED_SIZE);
        }
    ...
    TPM2_Packet* rsp, UINT16 cmdTag)
{
    ...
    {
        UINT64 clockMs = FWTPM_Clock_GetMs(ctx);
    ...
    int cmdSize, TPM2_Packet* rsp, UINT16 cmdTag)
{
    ...
    /* Parse inSensitive (TPM2B_SENSITIVE_CREATE) — skip data */
    if (rc == 0) {
        rc = FwParseSensitiveCreate(cmd, cmdSize, &userAuth,
            NULL, 0, NULL);
    }
    ...
    /* Hash the public template for cache lookup */
    if (rc == 0) {
        rc = wc_Sha256Hash((byte*)&inPublic.publicArea,
            (word32)sizeof(inPublic.publicArea), templateHash);
        if (rc != 0) {
            rc = TPM_RC_FAILURE;
        }
    }
    ...
    int cmdSize, TPM2_Packet* rsp, UINT16 cmdTag)
{
    ...
    if (rc == 0) {
        ...
        if ((saveHandle & 0xFF000000) == TRANSIENT_FIRST) {
            ...
            if (FwFindSession(ctx, saveHandle) == NULL) {
                rc = TPM_RC_HANDLE;
            }
            else {
    ...
    if (rc == 0) {
        /* Build opaque blob: magic(4) | version(4) | handle(4) | padding(4) */
        tmp32 = TPM2_Packet_SwapU32(FWTPM_CTX_MAGIC);
        XMEMCPY(blob + 0, &tmp32, 4);
        tmp32 = TPM2_Packet_SwapU32(FWTPM_CTX_VER);
        XMEMCPY(blob + 4, &tmp32, 4);
        tmp32 = TPM2_Packet_SwapU32(saveHandle);
        XMEMCPY(blob + 8, &tmp32, 4);
        XMEMSET(blob + 12, 0, 4);

        /* TPMS_CONTEXT: sequence(8) | savedHandle(4) | hierarchy(4) | blob */
        ctx->contextSeqCounter++;
        {
            UINT32 seqHi = (UINT32)(ctx->contextSeqCounter >> 32);
            UINT32 seqLo = (UINT32)(ctx->contextSeqCounter & 0xFFFFFFFFu);
        ...
        /* contextBlob as TPM2B: size(2) | data */
        TPM2_Packet_AppendU16(rsp, blobSz);
        TPM2_Packet_AppendBytes(rsp, blob, blobSz);
    ...
    int cmdSize, TPM2_Packet* rsp, UINT16 cmdTag)
{
    ...
    UINT16 blobSz = 0;
    byte blob[32];
    UINT32 magic = 0, version = 0, origHandle = 0;
    ...
    if (blobSz > (UINT16)sizeof(blob) || (blobSz > 0 && blobSz < 12)) {
        rc = TPM_RC_SIZE;
    }
    if (rc == 0 && blobSz > 0) {
        TPM2_Packet_ParseBytes(cmd, blob, blobSz);
    }

    /* Validate our blob magic and version */
    if (rc == 0) {
        XMEMCPY(&magic, blob + 0, 4);
        magic = TPM2_Packet_SwapU32(magic);
        XMEMCPY(&version, blob + 4, 4);
        version = TPM2_Packet_SwapU32(version);
        XMEMCPY(&origHandle, blob + 8, 4);
        origHandle = TPM2_Packet_SwapU32(origHandle);
    ...
    /* Context blob integrity - sequence/hierarchy unused in current
     * implementation (object remains in slot; blob is opaque handle ref) */
    ...
    if (rc == 0) {
        ...
        /* Verify the object/session still exists in our slot */
        if ((origHandle & 0xFF000000) == TRANSIENT_FIRST) {
            if (FwFindObject(ctx, origHandle) == NULL) {
                rc = TPM_RC_HANDLE;
            }
        }
        else if ((origHandle & 0xFF000000) == HMAC_SESSION_FIRST ||
                 (origHandle & 0xFF000000) == POLICY_SESSION_FIRST) {
            if (FwFindSession(ctx, origHandle) == NULL) {
                rc = TPM_RC_HANDLE;
            }
        }
        else {
    ...
    if (rc == 0) {
        /* Return same handle (object remains in slot) */
        TPM2_Packet_AppendU32(rsp, origHandle);
        FwRspFinalize(rsp, TPM_ST_NO_SESSIONS, TPM_RC_SUCCESS);
    }
    ...
    TPM2_Packet* rsp, UINT16 cmdTag)
{
    ...
    if (rc == 0) {
        ...
        wc_RNG_GenerateBlock(&ctx->rng, ctx->ownerSeed, FWTPM_SEED_SIZE);
        wc_RNG_GenerateBlock(&ctx->rng, ctx->endorsementSeed, FWTPM_SEED_SIZE);
...
/* Helper: parse TPM2B_SENSITIVE_CREATE (userAuth + optional data).
    ...
    UINT16* sensDataSize)
{
    ...
    if (rc == 0) {
        TPM2_Packet_ParseU16(cmd, &inSensSize);
    ...
    if (rc == 0) {
        TPM2_Packet_ParseU16(cmd, &userAuth->size);
        if (userAuth->size > sizeof(userAuth->buffer)) {
            rc = TPM_RC_SIZE;
        }
    }
    if (rc == 0 && userAuth->size > 0) {
        if (cmd->pos + userAuth->size > cmdSize) {
            rc = TPM_RC_COMMAND_SIZE;
        }
        if (rc == 0) {
            TPM2_Packet_ParseBytes(cmd, userAuth->buffer, userAuth->size);
        }
    }
    ...
    if (rc == 0) {
        TPM2_Packet_ParseU16(cmd, &dataSz);
        if (sensData != NULL) {
    ...
    if (rc == 0 && dataSz > 0) {
        if (cmd->pos + dataSz > cmdSize) {
            rc = TPM_RC_COMMAND_SIZE;
        }
        if (rc == 0) {
            if (sensData != NULL) {
                TPM2_Packet_ParseBytes(cmd, sensData, dataSz);
            }
            else {
                cmd->pos += dataSz; /* skip */
            }
    ...
    int cmdSize, TPM2_Packet* rsp, UINT16 cmdTag)
{
    ...
    FWTPM_DECLARE_BUF(plainSens, FWTPM_MAX_PRIVKEY_DER + 128);
    int plainSensSz = 0;
    UINT16 sensType = 0;
    UINT16 primeSz = 0;
    ...
    FWTPM_ALLOC_BUF(plainSens, FWTPM_MAX_PRIVKEY_DER + 128);
    ...
    if (rc == 0) {
        rc = TPM2_KDFa(parentNameAlg, seedBuf, seedSz,
    ...
    if (rc == 0) {
        rc = FwImportVerifyAndDecrypt(parentNameAlg,
            ...
            plainSens, (int)(FWTPM_MAX_PRIVKEY_DER + 128), &plainSensSz);
    }
    ...
    int cmdSize, TPM2_Packet* rsp, UINT16 cmdTag)
{
    ...
    if (rc == 0 && oldParent != NULL) {
        ...
        if (rc == 0) {
            rc = FwImportVerifyAndDecrypt(parentNameAlg,
                hmacKeyBuf, digestSz, aesKey, symKeySz,
                nameBuf, nameSz, dupBuf, dupSz,
                plainSens, FWTPM_MAX_PRIVKEY_DER + 128, &plainSensSz);
        }
        TPM2_ForceZero(seedBuf, sizeof(seedBuf));
        TPM2_ForceZero(aesKey, sizeof(aesKey));
        TPM2_ForceZero(hmacKeyBuf, sizeof(hmacKeyBuf));
    ...
    TPM2_ForceZero(plainSens, FWTPM_MAX_PRIVKEY_DER + 128);
    FWTPM_FREE_BUF(dupBuf);
    FWTPM_FREE_BUF(symSeedBuf);
    FWTPM_FREE_BUF(plainSens);
    return rc;
    ...
    if (rc == 0) {
        if (encScheme == TPM_ALG_OAEP) {
            ...
            outSz = wc_RsaPublicEncrypt_ex(message.buffer, message.size,
                ...
                WC_RSA_OAEP_PAD, wcHashType, WC_MGF1SHA256, NULL, 0);
        }
    ...
    int cmdSize, TPM2_Packet* rsp, UINT16 cmdTag)
{
    ...
    if (rc == 0) {
        ...
        {
            enum wc_HashType ht = FwGetWcHashType(hashAlg);
            wcHashType = (int)ht;
        }
...
{
    if (seq->isHmac) {
        wc_HmacFree(&seq->hmac);
    }
    else {
        wc_HashFree(&seq->hash, FwGetWcHashType(seq->hashAlg));
    }
    ...
    int cmdSize, TPM2_Packet* rsp, UINT16 cmdTag)
{
    ...
    if (rc == 0) {
        ...
        rc = wc_HmacSetKey(&seq->hmac, wcHashType,
            obj->privKey, (word32)obj->privKeySize);
        if (rc != 0) {
            rc = TPM_RC_FAILURE;
        }
    ...
    int cmdSize, TPM2_Packet* rsp, UINT16 cmdTag)
{
    ...
    if (rc == 0) {
        ...
        rc = wc_HashInit(&seq->hash, wcHash);
        if (rc != 0) {
            rc = TPM_RC_FAILURE;
        }
    }
    ...
    int cmdSize, TPM2_Packet* rsp, UINT16 cmdTag)
{
    ...
    if (rc == 0) {
        ...
        if (seq->isHmac) {
            rc = wc_HmacUpdate(&seq->hmac, dataBuf, dataSize);
        }
        else {
            rc = wc_HashUpdate(&seq->hash, FwGetWcHashType(seq->hashAlg),
                dataBuf, dataSize);
        }
        if (rc != 0) {
            rc = TPM_RC_FAILURE;
    ...
    int cmdSize, TPM2_Packet* rsp, UINT16 cmdTag)
{
    ...
    if (rc == 0) {
        if (seq->isHmac) {
            ...
            if (rc == 0) {
                rc = wc_HmacFinal(&seq->hmac, digest);
                if (rc != 0) {
                    rc = TPM_RC_FAILURE;
                }
            }
    ...
    const byte* policyRef, int policyRefSz, int hasRef)
{
    ...
    if (rc == 0) {
        wc_HashUpdate(hashCtx, wcHash,
            sess->policyDigest.buffer, sess->policyDigest.size);
        ccBuf[0]=(byte)(cc>>24); ccBuf[1]=(byte)(cc>>16);
        ccBuf[2]=(byte)(cc>>8);  ccBuf[3]=(byte)(cc);
        wc_HashUpdate(hashCtx, wcHash, ccBuf, 4);
    ...
    if (rc == 0) {
        ...
        if (sess->cpHashA.size > 0) {
            if (sess->cpHashA.size != cpHashSz ||
                XMEMCMP(sess->cpHashA.buffer, cpHashBuf, cpHashSz) != 0) {
                rc = TPM_RC_CPHASH;
            }
        }
    }
    ...
    if (rc == 0) {
        ...
        if (sess->nameHash.size > 0) {
            if (sess->nameHash.size != nameHashSz ||
                XMEMCMP(sess->nameHash.buffer, nameHashBuf,
                    nameHashSz) != 0) {
                rc = TPM_RC_CPHASH;
            }
        }
    ...
    if (rc == 0 && cpHashASz > 0) {
        if (sess->cpHashA.size > 0 &&
            (sess->cpHashA.size != cpHashASz ||
             XMEMCMP(sess->cpHashA.buffer, cpHashABuf, cpHashASz) != 0)) {
            rc = TPM_RC_CPHASH;
        }
        if (rc == 0) {
            sess->cpHashA.size = cpHashASz;
    ...
    byte* rspBuf, int* rspSize, int locality)
{
     ...
     * Sessions are not flushed — the calling tool manages its own sessions. */
    if (ctx->pendingClear) {
        ctx->pendingClear = 0;
        FwFlushAllObjects(ctx);
        FWTPM_NV_Save(ctx);
