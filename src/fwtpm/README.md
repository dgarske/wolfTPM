...
SPI/I2C for bare-metal integration. Implements 103 of 113 TPM 2.0 v1.38 commands
(91% coverage) with HAL abstractions for IO and NV storage portability.
...
```bash
src/fwtpm/fwtpm_server              # Listens on localhost:2321 (cmd) and :2322 (platform)
src/fwtpm/fwtpm_server --clear      # Delete NV state and start fresh
```
...
# Full test including tpm2-tools (290 tests)
...
### Separate Job: tpm2-tools (290 tests)
...
| Option | Description |
|--------|-------------|
| `--enable-fwtpm` | Build fwtpm_server alongside client library |
| `--enable-fwtpm-only` | Build only fwtpm_server (no client/examples) |
| `--enable-fuzz` | Build libFuzzer harness (requires CC=clang) |

### Feature Macros

| Macro | Effect |
|-------|--------|
| `FWTPM_NO_ATTESTATION` | Disable Quote, Certify, GetTime, MakeCredential, ActivateCredential |
| `FWTPM_NO_NV` | Disable NV RAM commands |
| `FWTPM_NO_POLICY` | Disable policy commands |
| `FWTPM_NO_CREDENTIAL` | Disable MakeCredential, ActivateCredential |
| `FWTPM_NO_DA` | Disable dictionary attack lockout commands |

### Spec Version Targeting

The fwTPM can be built targeting a specific TPM 2.0 Library Specification version.
This controls which commands are compiled in, using `--enable-fwtpm-spec=VERSION`.

| Option | Macro | Commands |
|--------|-------|----------|
| `--enable-fwtpm-spec=1.38` | `FWTPM_SPEC_V138` | Base v1.38 command set |
| `--enable-fwtpm-spec=1.59` | `FWTPM_SPEC_V159` | Adds MAC, CertifyX509, ACT |
| `--enable-fwtpm-spec=1.84` | `FWTPM_SPEC_V184` | Adds ECC_Encrypt/Decrypt, ReadOnlyControl, extended NV |
| `--enable-fwtpm-spec=1.85` | `FWTPM_SPEC_V185` | Adds PQC (ML-KEM, ML-DSA) commands |

Default: `FWTPM_SPEC_V138` (baseline). Spec version macros are cumulative -- v1.59
implies v1.38 support, v1.84 implies v1.59, etc.

```c
/* Example in fwtpm.h */
#ifdef FWTPM_SPEC_V185
    #define FWTPM_SPEC_V184
#endif
#ifdef FWTPM_SPEC_V184
    #define FWTPM_SPEC_V159
#endif
#ifdef FWTPM_SPEC_V159
    #define FWTPM_SPEC_V138
#endif
#ifndef FWTPM_SPEC_V138
    #define FWTPM_SPEC_V138  /* default baseline */
#endif
```

## TPM 2.0 Command Coverage

### Currently Implemented (103 commands)

The fwTPM implements 103 of 113 commands from the v1.38 baseline (91% coverage).
...
ContextSave, ContextLoad, ReadPublic, Clear, ClearControl, HierarchyControl,
...
#### v1.38 Baseline (10 missing commands)

##### Medium (moderate logic, builds on existing infrastructure) -- `FWTPM_SPEC_V138`

| Command | Spec Section | Difficulty | Notes |
|---------|-------------|------------|-------|
| `TPM2_ChangePPS` | 24.4 | Medium | Replace platform primary seed, flush platform objects. Regenerate ppSeed + update NV |
| `TPM2_ChangeEPS` | 24.5 | Medium | Replace endorsement primary seed, flush endorsement objects. Regenerate epSeed + update NV |
