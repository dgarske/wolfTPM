...
static int TisShmInit(void* ctx, FWTPM_TIS_REGS** regs)
{
    ...
    fd = open(FWTPM_TIS_SHM_PATH, O_CREAT | O_RDWR | O_TRUNC, 0660);
    if (fd < 0) {
    ...
    shm->semCmd = sem_open(FWTPM_TIS_SEM_CMD, O_CREAT | O_EXCL, 0660, 0);
    ...
    shm->semRsp = sem_open(FWTPM_TIS_SEM_RSP, O_CREAT | O_EXCL, 0660, 0);
