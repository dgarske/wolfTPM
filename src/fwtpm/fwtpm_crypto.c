    ...
    TPMI_ALG_HASH hashAlg, byte* proofOut, int proofSize)
{
    ...
    rc = TPM2_KDFa(hashAlg, seed, FWTPM_SEED_SIZE,
    ...
    const byte* sensData, int sensDataSz)
{
    ...
    /* Fill in totalSize */
    {
        UINT16 innerSize = (UINT16)(pos - innerStart);
    ...
    if (keyObj->pub.type == TPM_ALG_RSA) {
        ...
        if (rc == 0) {
            ...
            rc = wc_RsaPrivateDecrypt_ex(encSeedBuf, (word32)encSeedSz,
                ...
                WC_MGF1SHA256, (byte*)(uintptr_t)oaepLabel, oaepLabelSz);
            if (rc <= 0) {
        ...
        case TPM_ALG_RSA: {
            ...
            if (rc == 0) {
                if (pad == WC_RSA_PSS_PAD) {
                    ...
                    int mgf;
                    if (sig->signature.rsassa.hash == TPM_ALG_SHA384)
                        mgf = WC_MGF1SHA384;
                    else
                        mgf = WC_MGF1SHA256;
    ...
    byte* hmacKey, int hmacKeySz)
{
    ...
    kdfRc = TPM2_KDFa(TPM_ALG_SHA256, seed, seedSz,
