    ...
    if (FW_NV_APPEND_ONLY(hal) && hal->writeAlign > 1) {
        ...
        while (src < size) {
            ...
            if (ctx->nvGranuleFill == align) {
                ...
                if (rc != 0) {
                    /* Drop the partial granule so a later op re-anchors from
                     * nvWritePos instead of wedging on the alignment guard. */
                    ctx->nvGranuleFill = 0;
                    return TPM_RC_FAILURE;
                }
    ...
    const byte* value, UINT16 valueLen)
{
    ...
    /* Ensure room for what each append also writes: the trailing MAC
     * (byte-addressable) or the MAC checkpoint + alignment pad (append-only). */
 ...
 * rewrite cleanly if a torn trailing commit remains. Returns TPM_RC_SUCCESS
 * (loaded), TPM_RC_NV_UNINITIALIZED (regenerate), or a negative error. */
    ...
    word32* valueBufSzP)
{
    ...
    if (haveKey && lastMacEnd == 0) {
        /* Nothing authenticated: discard (caller regenerates fresh state). */
    ...
    if (lastMacEnd != 0) {
        ...
        committedEnd = scanPos;        /* no integrity key: trust the scan */
        ctx->nvWritePos = scanPos;
    }
    ...
    *valueBufSzP = valueBufSz;
    ...
    /* A torn commit after the last good checkpoint leaves un-erased cells that
     * block future appends; rewrite the recovered state cleanly. */
    if (rc == TPM_RC_SUCCESS && lastMacEnd != 0 && scanPos > lastMacEnd) {
        rc = FWTPM_NV_Save(ctx);
    }
...
int FWTPM_NV_Init(FWTPM_CTX* ctx)
{
    ...
    /* Append-only derives writePos by scanning (see FwNvInitAppendOnly). */
#ifdef WOLFTPM_FWTPM_NV_APPEND_ONLY
    if (rc == TPM_RC_SUCCESS && FW_NV_APPEND_ONLY(hal) &&
        hdr.magic == FWTPM_NV_MAGIC &&
        hdr.version == FWTPM_NV_VERSION) {
...
int FWTPM_NV_Save(FWTPM_CTX* ctx)
{
...
#ifdef WOLFTPM_FWTPM_NV_APPEND_ONLY
    /* Freshly erased: reset the pending granule; rewrite streams from 0. */
    ctx->nvGranuleBase = 0;
    ctx->nvGranuleFill = 0;
#endif
     ...
     * written after erase); byte-addressable updates the header in place. */
    if (rc == 0) {
#ifdef WOLFTPM_FWTPM_NV_APPEND_ONLY
        if (FW_NV_APPEND_ONLY(hal)) {
            rc = FwNvAppendCheckpoint(ctx);
